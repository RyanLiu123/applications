...
import com.huaqin.mmitest.sensor.AFMFunctionCallEx;
import com.huaqin.mmitest.sensor.FunctionReturn;
    ...
    public static int execCommandLocal(String[] command) throws IOException {
        ...
        try {
            ...
            if (proc.waitFor() == 0) {
                ...
                LogUtil.i(TAG, "exit value = " + proc.exitValue());
                sResultBuilder.append(ERROR);
                result = RESULT_FAIL;
            }
        } catch (InterruptedException e) {
        	LogUtil.i(TAG, "exe shell command InterruptedException: "
                    + e.getMessage());
            sResultBuilder.append(ERROR);
            ...
            if (null != bufferedReader) {
                try {
                	...
                	LogUtil.w(TAG, "close reader in finally block exception: " + e.getMessage());
                }
            }
        }
        return result;
    ...
    public static int execCommandOnServer(String[] command) throws IOException {
        ...
        if (ret) {
            ...
            if (output != null && output.startsWith(OPERATION_ERROR_PREFIX)) {
            	LogUtil.d(TAG, "error operation:" + output);
                sResultBuilder.delete(0, OPERATION_ERROR_PREFIX.length() + 1);
                return RESULT_FAIL;
            }
            return RESULT_SUCCESS;
        }
        LogUtil.d(TAG, "Function call start fail");
        sResultBuilder.append(ERROR);
        return RESULT_FAIL;
    }
}
